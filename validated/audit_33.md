...
**Intended Logic:**
According to the protocol's critical invariants, in `WHITELIST_ENABLED` state, ONLY whitelisted users can send/receive/burn iTRY tokens. [1](#0-0) 

The `_beforeTokenTransfer` hook should enforce this restriction uniformly across all token operations, including cross-chain minting when tokens arrive via LayerZero messages.

**Actual Logic:**
In `WHITELIST_ENABLED` mode, the validation logic is inconsistent:

**For cross-chain minting operations** (when LayerZero endpoint mints tokens to recipient): [2](#0-1) [3](#0-2) 

These checks only verify the recipient is NOT blacklisted, without checking if they ARE whitelisted.

**In contrast, normal transfers** correctly require ALL parties to be whitelisted: [4](#0-3) [5](#0-4) 
...
2. A non-whitelisted attacker coordinates with any whitelisted user possessing iTRY tokens on the opposite chain
3. The whitelisted user calls OFT `send()` function to bridge iTRY to the attacker's non-whitelisted address on the destination chain
4. LayerZero delivers the message, triggering `_credit()` → `_mint(attackerAddress, amount)` → `_beforeTokenTransfer(address(0), attackerAddress, amount)` with `msg.sender` = minter (LayerZero endpoint)
5. The `_beforeTokenTransfer` hook matches the minting condition, checking only `!blacklisted[to]` without verifying `whitelisted[to]`
6. The attacker receives iTRY tokens despite not being whitelisted, violating the protocol's documented access control invariant
