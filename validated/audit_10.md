...
After systematic validation against the Brix Money Protocol framework, this security claim is **VALID**.
...
However, `_beforeTokenTransfer` does NOT prevent minting shares to a clean receiver address when the caller has FULL_RESTRICTED, because during minting: `from = address(0)` (no role check applies) and `to = cleanReceiver` (no FULL_RESTRICTED role). This allows the bypass to succeed.
...
**Primary Fix:**
Add comprehensive role checks in the `_deposit` function to match documented behavior and align with `_withdraw` implementation:

In `src/token/wiTRY/StakediTry.sol`, function `_deposit`, modify line 247 to include FULL_RESTRICTED checks for both caller and receiver, matching the validation pattern used in the `_withdraw` function.

**Alternative Mitigation:**
Enforce mutual exclusivity in `addToBlacklist` to prevent users from holding both roles simultaneously. When upgrading from SOFT to FULL restriction, automatically revoke the SOFT role to prevent the dual-role exploitation scenario.

## Notes

**Critical Observations:**

1. **Inconsistent Validation:** The `_withdraw` function properly checks FULL_RESTRICTED for caller, receiver, and owner [5](#0-4) , while `_deposit` only checks SOFT_RESTRICTED [2](#0-1) . This inconsistency is the root cause.

2. **Role Management Flaw:** The `addToBlacklist` and `removeFromBlacklist` functions don't enforce mutual exclusivity [3](#0-2) , allowing users to hold both roles simultaneously and creating exploitable state transitions.

3. **Test Suite Gap:** Existing tests in `StakediTry.redistributeLockedAmount.t.sol` deposit FIRST then restrict [7](#0-6) , never testing whether FULL_RESTRICTED users can deposit. This test gap indicates the oversight was not caught during development.
...
**File:** test/StakediTry.redistributeLockedAmount.t.sol (L74-82)
```text
