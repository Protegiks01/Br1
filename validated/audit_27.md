...
After performing ruthless technical validation against the Brix Money Protocol codebase, I confirm this is a **legitimate High severity vulnerability**.
...
**Intended Logic:** 
The protocol's documented invariant states: "Only whitelisted user can send/receive/burn iTry tokens in a WHITELIST_ENABLED transfer state." [1](#0-0)  In `WHITELIST_ENABLED` state, the whitelist enforcement mechanism should ensure that ONLY whitelisted users can send, receive, or burn iTRY tokens.

**Actual Logic:** 
The `_beforeTokenTransfer` function contains an asymmetric validation pattern in the `WHITELIST_ENABLED` state:

The minting path only validates that the recipient is not blacklisted: [2](#0-1) 

However, burning and normal transfers require whitelist membership: [3](#0-2) 

This creates a one-way trap where tokens can be minted to a non-whitelisted wallet but can never leave.
...
4. **Tokens Locked**: iTryTokenOFTAdapter locks 1000 iTRY on hub chain [4](#0-3) 
5. **Mint on Spoke**: LayerZero endpoint calls `lzReceive` → `_credit` → `_mint`. The `_beforeTokenTransfer` check at line 160-161 passes because it only validates `!blacklisted[to]`, not `whitelisted[to]`. Alice receives 1000 iTRY on spoke.
...
Violates the whitelist enforcement invariant that in `WHITELIST_ENABLED` state, ONLY whitelisted users can send/receive/burn iTRY. The contract allows non-whitelisted users to receive via cross-chain minting, breaking the documented invariant.
...
- No automated recovery mechanism exists - `redistributeLockedAmount()` only works for blacklisted users [5](#0-4) 
...
Modify the minting validation in `WHITELIST_ENABLED` state to require whitelist membership at line 160:
...
Following the pattern used in `wiTryOFT.sol`, override the `_credit` function to redirect tokens to owner if recipient is not whitelisted. The `wiTryOFT` contract demonstrates this pattern for blacklist validation: [6](#0-5) 
...
1. **Documented Invariant Violation**: The README explicitly states that in `WHITELIST_ENABLED` state, only whitelisted users can receive tokens, which this implementation violates.

2. **Differential Analysis**: The `wiTryOFT` contract includes a `_credit` override that redirects tokens to the owner if the recipient is blacklisted, demonstrating awareness of the cross-chain validation issue. However, `iTryTokenOFT.sol` lacks equivalent protection for whitelist enforcement.

3. **Asymmetric Validation**: The minting path only checks `!blacklisted[to]` while all other operations require `whitelisted[...]` checks, creating a one-way trap.

4. **Recovery Limitation**: The `redistributeLockedAmount()` function only works for blacklisted users (requires `blacklisted[from]` to be true), meaning funds from non-whitelisted users cannot be recovered without first blacklisting them.

5. **Cross-Chain Trap**: Users bridging from hub have no way to know spoke-side requirements, making this a dangerous footgun that causes permanent fund loss.
