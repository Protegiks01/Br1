...
**Primary Fix:**
Add comprehensive role checks in the `_deposit` function to match the documented behavior and be consistent with `_withdraw`: [2](#0-1) 

The fix should check both `SOFT_RESTRICTED_STAKER_ROLE` and `FULL_RESTRICTED_STAKER_ROLE` for both caller and receiver addresses, similar to how `_withdraw` validates `FULL_RESTRICTED_STAKER_ROLE` for all participants.

**Alternative Mitigation:**
Enforce mutual exclusivity in `addToBlacklist` to prevent users from holding both roles simultaneously. When upgrading from SOFT to FULL restriction, automatically revoke the SOFT role. [3](#0-2) 
...
2. **Role Management Flaw:** The `addToBlacklist` and `removeFromBlacklist` functions don't enforce mutual exclusivity [7](#0-6) , allowing users to hold both roles simultaneously and creating exploitable state transitions.

3. **Documentation vs. Implementation:** The comment explicitly states FULL_RESTRICTED prevents "transfer, stake, or unstake" [1](#0-0) , but the implementation only enforces this for transfer and unstake, not stake (deposit).
...
**File:** src/token/wiTRY/StakediTry.sol (L126-143)
```text
