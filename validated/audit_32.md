...
The WHITELIST_ENABLED state exists specifically for regulatory compliance scenarios [1](#0-0) , and this bypass renders that control completely ineffective for cross-chain operations.

## Finding Description

**Location:** 
- `src/token/iTRY/crosschain/iTryTokenOFT.sol` in the `_beforeTokenTransfer` function
- `src/token/iTRY/iTry.sol` in the `_beforeTokenTransfer` function

**Intended Logic:** 
According to the protocol's documented invariants, in WHITELIST_ENABLED state, ONLY whitelisted users can send/receive/burn iTRY tokens [2](#0-1) . The `_beforeTokenTransfer` hook must enforce this restriction for ALL token operations, including cross-chain receives.
...
- **iTryTokenOFT.sol spoke chain**: The minting condition at line 160 checks only `!blacklisted[to]` [3](#0-2) 

- **iTry.sol hub chain**: The minting condition at line 201 checks only `!hasRole(BLACKLISTED_ROLE, to)` [4](#0-3) 

In stark contrast, normal transfers in WHITELIST_ENABLED mode correctly require ALL parties to be whitelisted:

- **iTryTokenOFT.sol normal transfers**: Line 168 requires `whitelisted[msg.sender] && whitelisted[from] && whitelisted[to]` [5](#0-4) 

- **iTry.sol normal transfers**: Lines 210-213 require all parties to have `WHITELISTED_ROLE` [6](#0-5) 

**Exploitation Path:**
1. Protocol administrators set `transferState` to `WHITELIST_ENABLED` on both chains to enforce compliance-only operations [7](#0-6) 
...
4. LayerZero delivers the message, triggering `_credit()` → `_mint()` → `_beforeTokenTransfer()` with `msg.sender` as the minter/endpoint (set at line 53 in iTryTokenOFT constructor) [8](#0-7) 
...
This vulnerability violates the critical documented invariant: "Only whitelisted user can send/receive/burn iTry tokens in a WHITELIST_ENABLED transfer state" [2](#0-1) 
...
- Any non-whitelisted address can receive iTRY through cross-chain transfers, rendering the whitelist feature ineffective for cross-chain operations
...
**User Impact**: Affects all users and the protocol's compliance posture. The WHITELIST_ENABLED state is specifically intended for regulatory compliance scenarios, and its bypass allows unrestricted distribution to non-approved addresses via cross-chain transfers.
